...
| 純手機模式 | 手機把 Facebook 通知直接送到電腦轉發 LINE，**電腦完全不連 Facebook**，帳號風險最低；代價是只抓得到有推播通知的內容（見 `PHONE_INGEST.md`）。**⚠️ 目前有未修的 P0，暫時不要開啟——見第 7.5 節前的警告** |
...
> ### ⚠️ 暫時不要開啟這兩個開關（2026-09-04）
>
> `trigger` 與 `phone_ingest` 這兩台伺服器目前有一個**未修的 P0**：未帶 token 的畸形 HTTP 請求
> （例如 `GET //[`）可以讓整個 watcher 行程結束（工單 WO-005）。`phone_ingest` 另有一條圖片解碼的
> 記憶體炸彈：68 bytes 的損毀 PNG 就可能把行程吃到被系統殺掉（工單 WO-012）。
>
> 兩者預設 `bind: 0.0.0.0`，**一旦開啟就對整個家用網路開放**，而且這兩條都不需要 token。
>
> **在 WO-005 與 WO-012 落地前，請維持 `enabled: false`**，用預設的 `interval` 模式做第一次真機試跑。
>
> 需要說清楚的是：這一節講的「帳號風險最低」**仍然成立** —— 那指的是 **Facebook 帳號**被風控的風險。
> 上面講的是**另一條風險軸**：本機服務被區網上的東西打掛。兩者不衝突，但目前後者還沒修。
...
如果家裡有一支閒置的 Android 手機，可以把它當成觸發器：手機上的官方 Facebook App 收到通知 → MacroDroid 打一個家用區網內的網址 → 電腦這時才去看一次 Facebook。時間點不再規律，新貼文通知也更快（數秒而不是最多 3 分鐘）。

載入次數會降多少**取決於你監看的粉專與社團有多熱鬧，不是固定值**：安全網巡邏的次數是固定的（15 分鐘間隔＝一天 96 次），觸發次數則等於通過節流的手機通知數。若兩個來源合計一天 10～30 則動態，總計約 110～130 次，比每 3 分鐘的 480 次少約 75%。反之若社團一天有上百則通知，觸發模式反而可能更多——這時要把 `trigger.min_interval_seconds` 調到 120～300 給觸發次數設上限。詳細算式與如何用 `npm run health` 量測實際次數，見 `PHONE_TRIGGER.md`。
