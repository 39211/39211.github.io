...
> **修正（2026-09-04，本機驗收席回報）**：原本寫「改回 150 跑 2 分鐘必見 `dead > 0`」是**錯的** ——
> 2 分鐘只產生約 18 則通知，遠低於 150，不可能觸發預算抑制。以下為可行版本。

把 `max_notifications_per_day` 暫時改成 **`5`**，跑一次 2 分鐘壓測：必須看到 `dead > 0` 且
「零 dead letter」gate FAIL。確認後改回 `100000`。這一步只是驗證，**不要 commit 突變後的值**。

（若要用 150 重現，需要跑滿約 17 分鐘才會累積到 151 則 —— 不值得，用 `budget=5` 即可。）
