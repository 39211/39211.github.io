...
> **狀態：已拍板，可派工。**
> 使用者於 2026-09-04 裁定採 **方案甲：群主豁免 + 隔日摘要**。
> 第 2 節的規格即為方案甲的完整寫法，直接照做。
...
**使用者 2026-09-04 裁定：方案甲 —— 群主豁免 + 隔日摘要。**

原始選項與取捨保留在下表存證。乙／丙／丁**不採用**，不要再回頭提。
...
7. **豁免的判定對象是「事件的作者」**，不是 target。逐事件判定：
   - `NEW_POST` / `EDITED_POST`：用 `PostEventPayload.author`
   - `NEW_COMMENTS`：群組內**任一則** item 的作者命中就整則豁免（截圖是一張，不能只送一半）
   - `PHONE_NOTIFICATION`：用通知的 `title`（手機端的發話者欄位），比對該來源的 `phone_ingest.notify_authors`
   - `VISUAL_CHANGE`：無作者資訊，**不豁免**
8. 豁免事件仍**計入** `notification_budget`（額度要誠實反映實際用量），只是不受上限擋。
9. 摘要的統計要把「因豁免而超額送出的則數」也列出來，讓使用者知道當天實際超用多少。
