...
雙開機把 RTC 當本地時間、VM 快照還原。

**用詞精確化（獨立驗證席回報，已採納）**：正確的描述是「去重視窗被延長了回撥的幅度」，
不是字面意義的「永久」。回撥幾秒 → 視窗多幾秒，無害；回撥數小時／數天且沒有再校正回來，
才會表現得像永久漏送。牆鐘追回來之後行為即恢復正常。
無論幅度大小，症狀都是：回傳 `duplicate`、HTTP 200、**不寫任何日誌**。
