...
一天實際載入 Facebook 的次數（觸發模式仍保留安全網巡邏，見下方「重要限制」）：

| 設定 | 安全網巡邏 | 加上觸發 | 一天合計 | 相對於每 3 分鐘 |
| --- | --- | --- | --- | --- |
| `interval` + 180 秒 | 480 次 | — | **約 480 次** | 基準 |
| `triggered` + 900 秒（15 分鐘） | 96 次 | 約 10～30 次 | **約 110～130 次** | 降約 75% |
| `triggered` + 1800 秒（30 分鐘） | 48 次 | 約 10～30 次 | **約 60～80 次** | 降約 85% |
...
**安全網間隔是速度與風險的取捨**：設 900 秒，留言最慢 15 分鐘到、一天約 120 次；設 1800 秒，留言最慢 30 分鐘到、一天約 70 次。先從 900 開始，若帳號常被要求驗證再調到 1800。
