...
export type ApprovalKind = "approved" | "forced" | "incomplete";

export type PublishStatus = "success" | "skipped" | "failed" | "uncertain" | "blocked";

export type AnalyticsStatus = "measured" | "unmeasured" | "total_only";

export type MockPublishOutcome =
  | "success"
  | "retryable"
  | "lost_response"
...
export class NonRetryablePublishError extends Error {
  readonly name = "NonRetryablePublishError";
...
export interface ShopState {
  ...
  posts: PublishRecord[];
  inFlight: Partial<Record<string, true>>;
}

export interface MockPlatform {
  nextOutcome: MockPublishOutcome;
  publishCalls: number;
  createdRemoteIds: string[];
}
...
export interface RegressionCase {
  id: string;
  group: "approval" | "retry" | "analytics";
  title: string;
  titleZh: string;
  run: () => void;
}

function flightKey(postId: string, platform: Platform): string {
  return `${postId}:${platform}`;
}

export function fingerprintFor(caption: string, mediaDigest?: string): string {
  return `${caption.trim()}::${mediaDigest ?? ""}`;
}

export function createShop(merchant = "示例洗衣店"): ShopState {
  return {
    merchant,
    paused: false,
    approvals: [],
...
export function createMockPlatform(nextOutcome: MockPublishOutcome = "success"): MockPlatform {
  return {
    nextOutcome,
    publishCalls: 0,
    createdRemoteIds: []
...
export function latestPublish(state: ShopState, postId: string, platform: Platform): PublishRecord | undefined {
  for (let index = state.posts.length - 1; index >= 0; index -= 1) {
    const row = state.posts[index];
    if (row && row.postId === postId && row.platform === platform) return row;
  }
  return undefined;
}

export function findApproval(state: ShopState, postId: string, platform: Platform): ApprovalRecord | undefined {
...
export function evaluateApproval(state: ShopState, draft: PostDraft, platform: Platform): ApprovalDecision {
  if (state.paused) {
...
export function approveDraft(
  state: ShopState,
  draft: PostDraft,
  options: { platform: Platform; kind?: ApprovalKind; note?: string } = { platform: "instagram" }
): ApprovalRecord {
  if (state.paused) {
...
function callMockPlatform(mock: MockPlatform, draft: PostDraft, platform: Platform): string {
  mock.publishCalls += 1;
  ...
  if (mock.nextOutcome === "lost_response") {
    ...
    throw new NonRetryablePublishError(
      `${platform} publish response was lost; the post may already be live. Not retrying.`
    );
  }
  if (mock.nextOutcome === "unknown_after_commit") {
    ...
    mock.createdRemoteIds.push(remoteId);
    throw new NonRetryablePublishError(
      `${platform} returned an unknown status after the commit point. Not retrying.`
    );
  }
  const remoteId = `remote-${platform}-${draft.id}-${mock.publishCalls}`;
  mock.createdRemoteIds.push(remoteId);
...
export function retryUntilPublished(
  state: ShopState,
  draft: PostDraft,
  platform: Platform,
  mock: MockPlatform,
...
export function aggregateAnalytics(days: AnalyticsReading[]): AnalyticsReading {
...
export function canJudgeCheckpoint(reading: AnalyticsReading): boolean {
  return reading.status === "measured" && reading.dataGaps.length === 0 && reading.value !== undefined;
}

export function sampleDraft(overrides: Partial<PostDraft> = {}): PostDraft {
  return {
    id: "slot-01",
    date: "2026-09-16",
    slot: 1,
...
export function buildRegressionCases(): RegressionCase[] {
  return [
    {
      id: "A01",
      group: "approval",
    ...
    },
    {
      ...
      run() {
        ...
        const ledger: { source_clicks?: number; source_clicks_status: string } = {
          source_clicks_status: reading.status
        };
        if (reading.status === "measured") ledger.source_clicks = reading.value;
...
export function runRegressionSuite(cases: RegressionCase[] = buildRegressionCases()): {
  passed: number;
  failed: number;
  results: SuiteResult[];
} {
...
export function runNamedScenarios(): {
  unapproved: PublishRecord;
  retry: { first: PublishRecord; second: PublishRecord; remotePosts: number };
